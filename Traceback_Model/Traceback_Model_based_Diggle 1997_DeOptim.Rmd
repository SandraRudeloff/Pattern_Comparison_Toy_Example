...
# calculation of flow
```{r}
flow <- hyman_model(0.4, 0.001,df_population_py, df_shops_py)
```
...
```{r}
# Test if the input is a character (string)
if (is.character(outbreak_data$list_outbreak_scenario_sizes)) {
  list_outbreak_scenario_sizes <- as.integer( unlist(strsplit(outbreak_data$list_outbreak_scenario_sizes, ",")))
} else {
  list_outbreak_scenario_sizes <- as.integer(outbreak_data$list_outbreak_scenario_sizes)
}
...
for (chain in unique(chains$chain)) {
